	...
	if player[owner].task = 1 then begin
		if Players[owner].Alive then begin
			case Players_OnGround(owner, false, 15) of
				1:	if player[owner].Wires > 0 then begin
						...
						for i:=1 to MAX_WIRES do
							...
							if Distance((wire[i].x1+wire[i].x2)*0.5, (wire[i].bundle[0].y + wire[i].bundle[1].y)*0.5, X, Y) < 15.0 then begin
...
procedure Wires_Process(MainCall: boolean);
var i, j: byte; ZombieOn: boolean;
	x, y: single;
	p: array[1..32] of boolean;
begin
	for i := 1 to MAX_WIRES do
		if wire[i].Active then begin
			wire[i].Active := Players[wire[i].owner].Active;
			if not wire[i].Active then continue;
			for j := 1 to MaxID do // hurt all zombies in wire range
				...
				if player[j].Zombie then begin
					...
					if x <= wire[i].x1 then
					if x >= wire[i].x2 then
					if Abs(wire[i].a*x + wire[i].b*y + wire[i].c) / wire[i].d < 25 then begin
						ZombieOn := true;
						Damage_DoRelative(j, wire[i].owner, WIREDMG, Wires);
						Players[j].SetVelocity(Players[j].VelX*0.5, Players[j].VelY*0.7);
						p[j] := true;
					end;
				end;
...
begin
	...
	if Result > 0 then begin
		hp := (single(wire[Result].hp) + single(wire[Result].DestructionStage)*WIREHITPOINTS/5.0)/WIREHITPOINTS;
		BigText_DrawMap(ID, 
